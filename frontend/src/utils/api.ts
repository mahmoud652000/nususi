...
const API_URL = import.meta.env.VITE_API_URL || "https://nususi-production.up.railway.app/api";
...
export const uploadAPI = {
  uploadBook: (formData: FormData) =>
    api.post("/upload/book", formData, { headers: { "Content-Type": "multipart/form-data" } }),
  uploadCover: (formData: FormData) =>
    api.post("/upload/cover", formData, { headers: { "Content-Type": "multipart/form-data" } }),
};
...
export const dashboardAPI = {
  ...
  updateBookStatus: (id: string, status: string) => api.put(`/dashboard/books/${id}/status`, { status }),
};
